...
void perdif_predict(data_t data, out_t out, ctrl_t ctrl, output_t outfiles)
{
	...
	for (int i = 0; i < data.num_models; i++)
	{
		...
		graphs[i] = generate_rec_graph(data.users_full, data.item_models[i], data.num_users, ctrl.bipartite);
	}
	...
	store_results(out.test_result, outfiles.outdir_pred, data.num_models, data.num_users, names);
...
void perdif_predict_smodel(struct pass2thread_t thread_data, int model_counter)
{
