...
void (*fit_theta[NUM_METHODS])(double *theta, d_mat_t P, double *g, int max_walk, double lambda, double *coef, bool rmse) = {&k_simplex,
																															 &single_best,
																															 &dictionary_single,
																															 &dictionary_single,
																															 &dictionary};
...
void perdif_mselect(data_t data, out_t out, ctrl_t ctrl, output_t outfiles)
{
	...
	csr_graph_t *graphs = (csr_graph_t *)malloc(data.num_models * sizeof(csr_graph_t));
	for (int i = 0; i < data.num_models; i++)
	{
		if (VERBOSE)
			printf("Building the training graphs: %d out of %d \n", i + 1, data.num_models);
		graphs[i] = generate_rec_graph(data.users, data.item_models[i], data.num_users, ctrl.bipartite);
	}
	...
	if (!ctrl.set_trend)
	{
		...
		yellow();
		printf("\nCross-Validation for the Selection of Item-Model and Max Walk-Length...");
		colorreset();
		...
		printf("The best models in CV are    : HR:%s ARHR:%s NDCG:%s\n", data.item_models[best_model_hr].name, data.item_models[best_model_arhr].name, data.item_models[best_model_ndcg].name);
		printf("The best scores in CV are    : HR:%f ARHR:%f NDCG:%f\n", best_hr, best_arhr, best_ndcg);
	...
	if (!ctrl.set_trend) // do not save made up trend values
		store_general_trends(out.general_trends, outfiles.outdir_theta, data.num_models, ctrl.max_walk, ctrl.set_trend, names);
...
//Function that trains thetas per user using the cross-validation items provided
void perdif_train(data_t data, out_t out, ctrl_t ctrl, output_t outfiles)
{
	...
	for (int i = 0; i < data.num_models; i++)
		names[i] = data.item_models[i].name;
	store_parameters(out.thetas, out.mus, outfiles.outdir_theta, data.num_models, names);
...
{
	//Unpack thread data
	metric_t *local_trend = thread_data.out.local_trend[thread_data.u_id][model_counter];
	...
	for (sz_long_t i = from_user; i < from_user + num_users; i++)
	{
		...
		if (report.yes && ((i - from_user) % (one_per_cent * report.every) == 0))
			...
			{
				if (i == from_user)
				{ //printf("General trends %d %% complete\n", report.shift + (int)(i - from_user) / (one_per_cent * report.every));
					printf("\nProgress: ");
					fflush(stdout);
				}
				else
...
void perdif_learn_smodel(struct pass2thread_t thread_data, int model_counter)
{
	...
	for (sz_long_t i = from_user; i < from_user + num_users; i++)
	{
		...
		for (int k = 0; k < ctrl.max_walk; k++)
		{
			if (k == 0)
			{
				...
				for (int j = 1; j < P.num_rows; j++)
					P.val[j][k] = (ctrl.bipartite) ? p[global_num_users + neg[j - 1]] : p[neg[j - 1]];
				// sanity check
				// double sum = 0.0;
			...
			// Populate k-th collumn of P
			P.val[0][k] = (ctrl.bipartite) ? p_plus[global_num_users + pos] : p_plus[pos];
			for (int j = 1; j < P.num_rows; j++)
				P.val[j][k] = (ctrl.bipartite) ? p_plus[global_num_users + neg[j - 1]] : p_plus[neg[j - 1]];
		...
		(*fit_theta[ctrl.which_dif_param])(thetas.val[i], P, g, ctrl.max_walk, ctrl.lambda, coefs, ctrl.rmse_fit);
