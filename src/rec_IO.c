...
static const char *dif_param_list[] = {
	"k-simplex",
	"single-best",
	"ppr",
	"hk",
...
static const char *metrics_list[] = {
	"HR",
	"ARHR",
	"NDCG"};
...
void parse_commandline_args(int argc, char **argv, cmd_args_t *args)
{
								   ...
								   long_options, &long_index)) != -1)
	{
		...
		{
		case 'd':
			args->ctrl.max_walk = atoi(optarg)+1; // the +1 is for using the max_walk as a barrier.
		...
		case 'q':
			//Input
			args->input.rating_mat = concat_thanos(3, DEFAULT_IN_DIR, optarg, "/R.csr");
			args->input.rating_mat_full = concat_thanos(3, DEFAULT_IN_DIR, optarg, "/R_full.csr");
			args->input.item_models_dir = concat_thanos(3, DEFAULT_IN_DIR, optarg, "/item_models");
			//Val
			args->input.val_mat = concat_thanos(3, DEFAULT_IN_DIR, optarg, "/CV.csr");
			// Test
			args->input.test_mat = concat_thanos(3, DEFAULT_OUT_DIR, optarg, "/TestSet.csr");
			//Output
			args->output.outdir_pred = concat_thanos(3, DEFAULT_OUT_DIR, optarg, "/predictions");
			args->output.outdir_theta = concat_thanos(3, DEFAULT_OUT_DIR, optarg, "/model_parameters");
			args->output.outdir_val = concat_thanos(3, DEFAULT_OUT_DIR, optarg, "/val_workspace");
			break;
			exit(EXIT_FAILURE);
		}
	}
...
data_t load_data(input_t input, ctrl_t *ctrl)
{
	...
	data.users_full = read_rating_matrix_csr(&test_dimension, input.rating_mat_full);
	printf("Done!\n");
	...
	printf("Validation-Set...");fflush(stdout);
	load_v(input.val_mat, &data.cv_pos, &data.cv_neg);
	printf("Done!\n");
	printf("Test-Set...");fflush(stdout);
	load_v(input.test_mat, &data.test_pos, &data.test_neg);
	printf("Done!\n");

	if (data.test_neg.num_rows != data.cv_neg.num_rows || data.test_neg.num_cols != data.cv_neg.num_cols)
	{
		red();
...
static void load_v(char *filename, sz_long_t **v_pos, i_mat_t *v_neg)
{
	...
	while (fgets(line, len, file) != NULL)
	{
		...
		{
			memcpy(v_neg->val[count_rows - 1], v_inds, count_cols * sizeof(sz_long_t));
		}
		free(v_inds);
		count_rows++;
	}
...
//Reads feedback matrix in csr format from file and converts it to an array of users
//Also returns the total number of users
user_t *read_rating_matrix_csr(sz_long_t *usr_count, char *filename)
{
	...
	*usr_count = 0;
	...
	while (fgets(line, len, file) != NULL)
	{
		users[*usr_count].items = csr_line_to_ind(line, &users[*usr_count].num_items, false);
...
csr_graph_t read_item_graph_csr(char *filename, char *modelname)
{
	...
	while (fgets(line, len, file) != NULL)
	{
		sz_med_t num_item_edges = csr_read_line(line, &item_graph.csr_value[item_graph.nnz], &item_graph.csr_column[item_graph.nnz]);
		item_graph.csr_row_pointer[item_graph.num_nodes + 1] = item_graph.csr_row_pointer[item_graph.num_nodes] + num_item_edges;
...
{
	*num_models = 0;
	csr_graph_t *models = (csr_graph_t *)malloc(MAX_ITEM_MODELS * sizeof(csr_graph_t));
	...
	if ((dir = opendir(dirname)) != NULL)
	{
		...
		{
			if (strstr(ent->d_name, ".model") != NULL) // check if the listed file contains the ".model" substring
			{
...
// Numbers are delimited by space and every second number is ignored (is always 1)
//If vals=true it loads the values of non zero entries
//If vals = false it loads the indices
static sz_long_t *csr_line_to_ind(char *line, sz_med_t *num_entries, bool vals)
{
...
static sz_med_t csr_read_line(char *line, long double *value, sz_long_t *column)
{
...
void store_general_trends(metric_t **general_trends, char *outdir, int num_models, int max_walk, bool set_trend, const char **names)
{
	...
	for (int i = 0; i < num_models; i++)
	{
		...
		for (int k = 0; k < max_walk; k++)
			fprintf(file, "%0.4f\t%0.4f\t%0.4f\n", general_trends[i][k].HR, general_trends[i][k].ARHR, general_trends[i][k].NDCG);

		if (set_trend)
			fprintf(file, "WARNING: Above results were manually set using --set_trend option\n");
		fclose(file);
...
void store_parameters(d_mat_t *thetas, d_mat_t *mus, char *outdir, int num_models, const char **names)
{
	if (VERBOSE)
	{
...
void store_results(metric_t **result, char *outdir, int num_models, sz_long_t num_users, const char **names)
{
	...
	for (int j = 0; j < num_models; j++)
	{
		...
		{
			fprintf(file, "%d\t%0.4f\t%0.4f\t%0.4f\n", (int)i, result[j][i].HR, result[j][i].ARHR, result[j][i].NDCG);
