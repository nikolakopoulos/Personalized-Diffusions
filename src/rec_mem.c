...
void d_mat_resize(d_mat_t *mat, sz_long_t new_num_rows, sz_long_t new_num_cols)
{
	sz_long_t num_rows = mat->num_rows;
	if (new_num_rows > num_rows)
	{
		for (sz_long_t i = 0; i < num_rows; i++)
			mat->val[i] = (double *)realloc(mat->val[i], new_num_cols * sizeof(double));
...
// Allocate integer matrix
void i_mat_resize(i_mat_t *mat, sz_long_t new_num_rows, sz_long_t new_num_cols)
{
	sz_long_t num_rows = mat->num_rows;
	if (new_num_rows > num_rows)
	{
		for (sz_long_t i = 0; i < num_rows; i++)
			mat->val[i] = (sz_long_t *)realloc(mat->val[i], new_num_cols * sizeof(sz_long_t));
		for (sz_long_t i = num_rows; i < new_num_rows; i++)
	...
	mat->val = (sz_long_t **)realloc(mat->val, new_num_rows * sizeof(sz_long_t *));
...
out_t output_init(data_t data, ctrl_t ctrl)
{
	...
	output.mus = (d_mat_t *)malloc(data.num_models * sizeof(d_mat_t)); // added for mus
	output.general_trends = (metric_t **)malloc(data.num_models * sizeof(metric_t *));
	output.test_result = (metric_t **)malloc(data.num_models * sizeof(metric_t *));

	for (int i = 0; i < data.num_models; i++)
	{
		output.thetas[i] = d_mat_init(data.num_users, ctrl.max_walk);
		output.mus[i] = d_mat_init(data.num_users, ctrl.max_walk-1); // mus are less than θs
		output.general_trends[i] = (metric_t *)malloc(ctrl.max_walk * sizeof(metric_t));
		output.test_result[i] = (metric_t *)malloc(data.num_users * sizeof(metric_t));
		for (int k = 0; k < ctrl.max_walk; k++)
			output.general_trends[i][k] = (metric_t){.HR = 0.0, .ARHR = 0.0, .NDCG = 0.0};
	}
	...
	if (ctrl.save_vals)
	{
		...
		for (int i = 0; i < data.num_models; i++)
		{
			...
			output.test_pred[i] = d_mat_init(data.num_users, data.test_neg.num_rows + 1);			
		}
	}

	output.local_trend = init_local_trend(ctrl.usr_threads, data.num_models, ctrl.max_walk);
