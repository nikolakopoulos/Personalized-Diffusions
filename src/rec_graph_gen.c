...
static csr_graph_t glue(user_t *users, csr_graph_t M_I, sz_long_t num_users) {
  ...
  csr_realloc(&graph, 2 * num_ratings + num_users + M_I.nnz,
              M_I.num_nodes + num_users);
  ...
  for (sz_long_t i = 0; i < num_users; i++) {
    ...
    for (sz_long_t j = 0; j < users[i].num_items; j++) {
      ...
      if (items[users[i].items[j]].num_users > itm_buff_size) {
        ...
        sz_long_t *temp_item = (sz_long_t *)malloc(
            (items[users[i].items[j]].num_users - 1) * sizeof(sz_long_t));
        memcpy(temp_item, items[users[i].items[j]].users,
               items[users[i].items[j]].num_users * sizeof(sz_long_t));
        items[users[i].items[j]].users =
            realloc(items[users[i].items[j]].users,
                    items[users[i].items[j]].num_users * sizeof(sz_long_t));
        memcpy(items[users[i].items[j]].users, temp_item,
               items[users[i].items[j]].num_users * sizeof(sz_long_t));
        free(temp_item);
      }
      items[users[i].items[j]].users[items[users[i].items[j]].num_users - 1] =
          i;
    }
  }
  ...
  for (sz_long_t i = 0; i < M_I.num_nodes; i++) {
    ...
    items[i].users =
        realloc(items[i].users, items[i].num_users * sizeof(sz_long_t));
  }
  ...
  for (sz_long_t i = num_users; i < graph.num_nodes; i++) {
    ...
    // Add row of M_I
    for (sz_long_t j = M_I.csr_row_pointer[k]; j < M_I.csr_row_pointer[k + 1];
         j++) {
