...
csr_graph_t csr_deep_copy_and_scale(csr_graph_t graph, double scale) {
  ...
  // CSR matrix with three arrays, first is basically a dummy for now since
  // networks I have are unweighted. However they will be weighted as sparse
  // stochastic matrices so values will be needed
  ...
  graph_temp.csr_row_pointer =
      (sz_long_t *)malloc((graph.num_nodes + 1) * sizeof(sz_long_t));
  ...
  memcpy(graph_temp.csr_row_pointer, graph.csr_row_pointer,
         (graph.num_nodes + 1) * sizeof(sz_long_t));

  memcpy(graph_temp.degrees, graph.degrees,
         graph.num_nodes * sizeof(sz_long_t));

  memcpy(graph_temp.csr_column, graph.csr_column,
         graph.nnz * sizeof(sz_long_t));
...
csr_graph_t csr_deep_copy(csr_graph_t graph) {
  ...
  memcpy(graph_temp.csr_value, graph.csr_value,
         graph.nnz * sizeof(long double));
...
csr_graph_t csr_transpose(csr_graph_t graph) {
  ...
  csr_graph_t graph_trans =
      edgelist_to_csr(edge_list, weights, graph.num_nodes, graph.nnz);
...
void csr_add_diagonal(csr_graph_t *graph, double *diag) {
  ...
  for (sz_long_t i = 0; i < graph->num_nodes; i++) {
    ...
    for (sz_long_t j = 0; j < graph->degrees[i] - 1; j++) {
      graph->csr_column[graph->csr_row_pointer[i] + j] =
          graph_temp.csr_column[graph_temp.csr_row_pointer[i] + j];
      graph->csr_value[graph->csr_row_pointer[i] + j] =
          graph_temp.csr_value[graph_temp.csr_row_pointer[i] + j];
    }
    graph->csr_column[graph->csr_row_pointer[i] + graph->degrees[i] - 1] = i;
    graph->csr_value[graph->csr_row_pointer[i] + graph->degrees[i] - 1] =
        diag[i];
  }
